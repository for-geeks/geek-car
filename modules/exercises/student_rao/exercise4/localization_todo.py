...
class Exercise(object):
	...
	while not cyber.is_shutdown():
	    time.sleep(0.05)
	    self.localization_with_odometer_calculation(self.speed, self.yaw, 0.05);
	...
	for i in range(0, len(data.tag)):
		...
		Rt = Rt.reshape((3,3),order='C')		
	        Tre = self.calculate_pos_by_apriltag(Rt, T)
    ...
    def localization_with_odometer_calculation(self, speed, yaw, delta_t):
	#please calculate predict.x and predict.z by speed and yaw and delta_t
