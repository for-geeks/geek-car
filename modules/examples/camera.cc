...
std::string get_tegra_pipeline(int width, int height, int fps) {
  ...
  return "nvarguscamerasrc ! 'video/x-raw(memory:NVMM),width=1024, height=768, framerate=120/1, format=NV12' ! nvvidconv ! nvegltransform ! nveglglessink -e";
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    while (apollo::cyber::OK()) {
      ...
      auto m_size = origin_image.rows * origin_image.cols * origin_image.elemSize(); 
