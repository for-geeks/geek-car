...
static void umb_initdone(void *param) { (void)param; }
...
UmbStandard::UmbStandard(std::map<umb_cb_recv, std::pair<int, int>> sub_list,
  ...
  while (it != sub_list.end()) {
    ...
    *inst_id_recv = counter;
    ...
    umb_sub_adv_reg(inst_id_recv, (umb_domain_id_t)(it->second).first,
  ...
  while (it_pub != pub_list.end()) {
    ...
    *inst_id_recv = counter;
    inst_list.push_back(inst_id_recv);
    umb_pub_adv_reg(inst_id_recv, (umb_domain_id_t)(it_pub->second).first,
...
void UmbStandard::send_umb_messeage(umb_inst_id_t inst_id, void *msg,
                                    unsigned len) {
  umb_msgbuff_t data;
  umb_errcode_e umb_ret = UMB_ERRCODE_OK;
