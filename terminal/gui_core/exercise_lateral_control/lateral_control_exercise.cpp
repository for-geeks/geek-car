...
static int obs_cb(umb_inst_id_t inst_id_in, umb_msgbuff_t *data,
                  umb_timestamp_t *tm, void *param) {
  // std::cout << data->len << std::endl;
  if (lateral_control_exercise_ptr != nullptr) {
    ((lateral_control_exercise *)lateral_control_exercise_ptr)
        ->update_obs((char *)data->buff, data->len);
  }
  return 0;
}

static int pose_cb_recv(umb_inst_id_t inst_id_in, umb_msgbuff_t *data,
                        umb_timestamp_t *tm, void *param) {
  // std::cout << data->len << std::endl;
  if (lateral_control_exercise_ptr != nullptr) {
    ((lateral_control_exercise *)lateral_control_exercise_ptr)
        ->update_pose((unsigned char *)data->buff, data->len);
  }
  return 0;
}

static int trajectory_feedback_cb(umb_inst_id_t inst_id_in, umb_msgbuff_t *data,
                                  umb_timestamp_t *tm, void *param) {
  // std::cout << data->len << std::endl;
  if (lateral_control_exercise_ptr != nullptr) {
    ((lateral_control_exercise *)lateral_control_exercise_ptr)
        ->update_trajectory((char *)data->buff, data->len);
  }
  return 0;
}

static int chassie_feedback_cb(umb_inst_id_t inst_id_in, umb_msgbuff_t *data,
                               umb_timestamp_t *tm, void *param) {
  // std::cout << data->len << std::endl;
  if (lateral_control_exercise_ptr != nullptr) {
    ((lateral_control_exercise *)lateral_control_exercise_ptr)
        ->update_chassie_info((char *)data->buff, data->len);
  }
  return 0;
}

lateral_control_exercise::lateral_control_exercise(QWidget *parent)
    : QWidget(parent), ui(new Ui::lateral_control_exercise) {
  ui->setupUi(this);
  qtimer = new QTimer();
  ...
  sub_list.insert(
      std::make_pair(trajectory_feedback_cb, std::make_pair(1035, 10)));
  sub_list.insert(std::make_pair(obs_cb, std::make_pair(1035, 12)));
  // pub_list.insert(std::make_pair(std::string("raw_image"),
  // std::make_pair(1035, 1)));
  umb_center = new UmbStandard(sub_list, pub_list);
  scene = new CustomScene();
  scene_lane = new CustomScene();
  scene_lane->s_steer_pic_item->~QGraphicsItem();
  scene->setSceneRect(0, 0, ui->graphicsView_pos->width() * 5,
                      ui->graphicsView_pos->height() * 5);
  ui->graphicsView_pos->setScene(scene);
  scene_lane->setSceneRect(0, 0, ui->graphicsView_lane->width(),
                           ui->graphicsView_lane->height());
  ui->graphicsView_lane->setScene(scene_lane);
  scene_lane->mesh_init();
  scene->mesh_init();
  memset(trajectory_pts, 0, 50 * sizeof(Point2d_s));
  // scene_lane->s_steer_pic_item->setPos(-22 +
  // scene_lane->sceneRect().center().x(), -70 +
  // scene_lane->sceneRect().center().y());
}

void lateral_control_exercise::update_pose(unsigned char *buff,
                                           unsigned int length) {
...
void lateral_control_exercise::_log_trajectory(void) {
  ...
  double yaw =
      atan((q1 * q2 - q0 * q3) * 2 / (q0 * q0 + q1 * q1 - q2 * q2 - q3 * q3));
  ...
  if (ui->checkBox_record->isChecked()) {
    if (_distance_2d(trajectory, last_trajectory) >= 0.05) {
      ...
      traj_vec.push_back(
          std::make_pair(300 * trajectory.x + scene->sceneRect().center().x(),
...
void lateral_control_exercise::_timer_update(void) {
  ...
  q0 = pose_reader.rotation.w;
  double yaw =
      atan2((q1 * q2 - q0 * q3) * 2, q0 * q0 + q1 * q1 - q2 * q2 - q3 * q3);
  // trajectory.x =
  ...
  ui->graphicsView_pos->horizontalScrollBar()->setValue(
      300 * point.x + scene->sceneRect().center().x() -
      ui->graphicsView_pos->width() / 2);
  ui->graphicsView_pos->verticalScrollBar()->setValue(
      300 * point.y + scene->sceneRect().center().y() -
...
double lateral_control_exercise::_distance_2d(trajectory_t t1,
                                              trajectory_t t2) {
...
std::vector<std::pair<float, float>> lateral_control_exercise::_sense_points(
    void) {
  ...
  if (((traj_vec.size() > (min_index + 20))) && (min_index != 9999)) {
    for (it = traj_vec.begin() + min_index;
         it != traj_vec.begin() + min_index + 20;) {
...
void lateral_control_exercise::_calculate_lane(laneinfo_t *lane) {
  ...
  // steer = 4 * atan(fitter.v[2]) * 180 / M_PI + 100 * 4 * (0 + fitter.v[3]) +
  // 100 * fitter.v[1]; steer = 100 * fitter.v[1]; 4 * atan(fitter.v[2]) * 180 /
  // M_PI + 100 * 4 * (0 + fitter.v[3]) +
  ...
  for (int i = 0; i < 50; i++) {
    if (fabs(trajectory_pts[i].y) > 0.01) {
      traj_vec_planning.push_back(
          std::make_pair(trajectory_pts[i].y, -trajectory_pts[i].x));
    }
  }
  ...
  for (int i = 0; i < list.size(); i++) {
    ...
    if (list.at(i)->type() == test.type()) {
      _global2vehicle(list.at(i)->pos().x(), list.at(i)->pos().y(), &x, &y);
  ...
  // float x_tar =
  // std::cout << target.obs_points.y << " " << (target.obs_points.y > 0.0
  // <<std::endl;
  if (target.obs_points.y > 0.0) {
    if (traj_vec_planning.size()) {
      steer =
          57 *
          atan2(
              2 * traj_vec_planning.at(traj_vec_planning.size() / 2).second *
                  0.313,
              (traj_vec_planning.at(traj_vec_planning.size() / 2).first *
                   traj_vec_planning.at(traj_vec_planning.size() / 2).first +
               traj_vec_planning.at(traj_vec_planning.size() / 2).second *
                   traj_vec_planning.at(traj_vec_planning.size() / 2).second));
    }
  } else {
    if (out_point.size()) {
      steer = 1 * atan(fitter.v[2]) * 180 / M_PI + 100 * 1 * (0 + fitter.v[3]) +
              -5 +
              57 * atan2(2 * out_point.at(out_point.size() - 10).second * 0.313,
                         (out_point.at(out_point.size() - 10).first *
                              out_point.at(out_point.size() - 10).first +
                          out_point.at(out_point.size() - 10).second *
                              out_point.at(out_point.size() - 10).second));
    }
  }
  if ((steer) >= 60) {
  ...
  if ((fabs(target.end_point.y) > 0.01) && (fabs(target.end_point.y) < 1)) {
    umb_center->send_umb_messeage(umb_center->pub_map["target_obs"], &target,
                                  sizeof(target));
  }
}

std::vector<std::pair<float, float>>
lateral_control_exercise::_global2vehicle_asvec(
...
void lateral_control_exercise::_global2vehicle(float x, float y, float *x_trans,
                                               float *y_trans) {
  float y_temp = (22 - x + scene->s_steer_pic_item->pos().x()) / 300.0;
  float x_temp = (70 - y + scene->s_steer_pic_item->pos().y()) / 300.0;
  float tempy = x_temp * sin(scene->s_steer_pic_item->rotation() * M_PI / 180) +
                y_temp * cos(scene->s_steer_pic_item->rotation() * M_PI / 180);
  float tempx = x_temp * cos(scene->s_steer_pic_item->rotation() * M_PI / 180) -
                y_temp * sin(scene->s_steer_pic_item->rotation() * M_PI / 180);
  *y_trans = tempy;
  *x_trans = tempx;
}

std::vector<std::pair<float, float>>
lateral_control_exercise::_vehicle2global_asvec(
...
void lateral_control_exercise::_vehicle2global(float x, float y, float *x_trans,
                                               float *y_trans, bool mode) {
  float centerx_trans =
      x * cos(scene->s_steer_pic_item->rotation() * -M_PI / 180) -
      y * sin(scene->s_steer_pic_item->rotation() * -M_PI / 180);
  float centery_trans =
      x * sin(scene->s_steer_pic_item->rotation() * -M_PI / 180) +
      y * cos(scene->s_steer_pic_item->rotation() * -M_PI / 180);
  *x_trans = 22 + scene->s_steer_pic_item->pos().x() + (-centery_trans * 300);
  *y_trans = 70 + scene->s_steer_pic_item->pos().y() + (-centerx_trans * 300);
}
  ...
  if (number == 8) {
    for (int j = 0; j < 2; j++) {
      for (int i = 0; i < distance / step; i++) {
        ...
        traj.push_back(trajectory);
        traj_vec.push_back(std::make_pair(
            300 * trajectory.x + scene->sceneRect().center().x(),
            300 * trajectory.y + scene->sceneRect().center().y()));
      }
